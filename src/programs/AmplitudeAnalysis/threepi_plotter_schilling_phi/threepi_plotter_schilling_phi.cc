...
typedef ThreePiPlotGeneratorSchillingPhi PlotGen;
...
int main( int argc, char* argv[] ){
  ...
  if (argc < 2){
    ...
    cout << "\tthreepi_schilling_plotter_phi <results file name> -o <output file name>" << endl << endl;
    return 0;
  }

  bool showGui = false;
  string outName = "threepi_schilling_plot_phi.root";
  ...
  for (unsigned int isum = 0; isum <= sums.size(); isum++){
   ...
   cout << "Looping over input data" << endl;
    ...
    for (unsigned int iplot = 0; iplot < PlotGenerator::kNumTypes; iplot++){
      ...
      for (unsigned int ivar  = 0; ivar  < ThreePiPlotGeneratorSchillingPhi::kNumHists; ivar++){
  ...
  ofstream outfile;
  outfile.open( "threepi_schilling_phi_fitPars.txt" );
