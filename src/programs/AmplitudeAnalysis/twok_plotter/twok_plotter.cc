...
typedef TwoKPlotGenerator PlotGen;
...
int main( int argc, char* argv[] ){
  ...
  if (argc < 2){
    ...
    cout << "\ttwok_plotter <results file name> -o <output file name>" << endl << endl;
    return 0;
  }

  bool showGui = false;
  string outName = "twok_plot.root";
  ...
  for (unsigned int isum = 0; isum <= sums.size(); isum++){
    ...
    for (unsigned int iplot = 0; iplot < PlotGenerator::kNumTypes; iplot++){
      ...
      for (unsigned int ivar  = 0; ivar  < TwoKPlotGenerator::kNumHists; ivar++){

        // set unique histogram name for each plot (could put in directories...)
        string histname =  "";
        if (ivar == TwoKPlotGenerator::k2PiMass)  histname += "M2pi";
	else if (ivar == TwoKPlotGenerator::kPiPCosTheta)  histname += "cosTheta";
        else if (ivar == TwoKPlotGenerator::kPhi)  histname += "Phi";
        else if (ivar == TwoKPlotGenerator::kphi)  histname += "phi";
        ...
        else if (ivar == TwoKPlotGenerator::kKmLabTheta) histname += "KmLabTheta";
        else if (ivar == TwoKPlotGenerator::kKmLabPhi) histname += "KmLabPhi";
        else if (ivar == TwoKPlotGenerator::kpKm) histname += "pKm";
