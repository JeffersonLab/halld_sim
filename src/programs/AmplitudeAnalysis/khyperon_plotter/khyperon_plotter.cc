...
typedef KHyperonPlotGenerator PlotGen;
...
int main( int argc, char* argv[] ){
  ...
  if (argc < 2){
    ...
    cout << "\tkhyperon_plotter <results file name> -o <output file name>" << endl << endl;
    return 0;
  }

  bool showGui = false;
  string outName = "khyperon_plot.root";
  ...
  for (unsigned int isum = 0; isum <= sums.size(); isum++){
    ...
    for (unsigned int iplot = 0; iplot < PlotGenerator::kNumTypes; iplot++){
      ...
      for (unsigned int ivar  = 0; ivar  < KHyperonPlotGenerator::kNumHists; ivar++){

        // set unique histogram name for each plot (could put in directories...)
        string histname =  "";
        if (ivar == KHyperonPlotGenerator::kHyperonMass)  histname += "MHyperon";
	else if (ivar == KHyperonPlotGenerator::kCosThetax)  histname += "cosThetaX";
	else if (ivar == KHyperonPlotGenerator::kCosThetay)  histname += "cosThetaY";
	else if (ivar == KHyperonPlotGenerator::kCosThetaz)  histname += "cosThetaZ";
	else if (ivar == KHyperonPlotGenerator::kCosThetaHyp) histname += "cosThetaHyp";
	else if (ivar == KHyperonPlotGenerator::kphiHyp) histname += "phiHyp";
        else if (ivar == KHyperonPlotGenerator::kPhiHyp)  histname += "Phi";
        //else if (ivar == KHyperonPlotGenerator::kt)  histname += "t";
        else continue;
  ...
  // ************************
  // retrieve fit parameters for plotting
  // ************************
