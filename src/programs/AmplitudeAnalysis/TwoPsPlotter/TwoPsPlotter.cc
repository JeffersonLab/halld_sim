...
struct ProgramOptions{
    ...
    std::string outName = "TwoPs_plot.root";
    bool showGui = false;
    bool mcGen = false;
    bool bkgFile = false;
    bool sFile = false;
...
// Note: this follow standard naming conventions for the two_ps amplitudes
// this means that:
...
ReflSum classifyReflSum(const std::string& sumName){
    if (sumName.find("ReZ_1-P") != std::string::npos ||
        sumName.find("ImZ_1+P") != std::string::npos){
        return ReflSum::Positive;  
    }
    else if (sumName.find("ReZ_1+P") != std::string::npos ||
             sumName.find("ImZ_1-P") != std::string::npos){
...
AmplitudeRegistry buildRegistry(std::unique_ptr<twops_PlotGen>& plotResults){    
...
void applyConfig(const PlotConfig& newConfig, const AmplitudeRegistry& reg, 
                  std::unique_ptr<twops_PlotGen>& plotResults){
...
// This function creates the plots that are assigned in TwoPsPlotGenerator
// Note: 'singleData' and 'singleBkg' are used to ask the question "has this been plotted already?"
void generatePlots(ProgramOptions& options, std::unique_ptr<twops_PlotGen>& plotResults,
                   const  std::string& reactionName,
                   const TFile& plotFile, bool& singleData, bool& singleBkg,
    ...
    for (unsigned int iType = 0; iType < PlotGenerator::kNumTypes; ++iType){
        ...
        if (iType == PlotGenerator::kBkgnd){
            if(!singleBkg) singleBkg = true;
            else if(singleBkg) continue;  // only plot bkg once
        }
        ...
        switch (iType) {
            ...
            case PlotGenerator::kBkgnd:   dataType = "bkg"; break;
            default: continue;
        }

        // Loop over all histograms defined in TwoPsPlotGenerator
        // The loop variable should be defined using the first entry in the 
        // enum, in this case kMass12 (change if needed). The last one is 
        // kNumHists and this one should not change
        for (TwoPsPlotGenerator::Hist_index ivar = TwoPsPlotGenerator::kFirstHist;
            ivar < TwoPsPlotGenerator::kNumHists;
            ivar = static_cast<TwoPsPlotGenerator::Hist_index>(ivar + 1)){
            ...
            // and the translation table are defined in TwoPsPlotGenerator 
            std::string histName = TwoPsPlotGenerator::numToString(ivar);

            histName += "_" + dataType;
            if(!(iType==PlotGenerator::kData) && !(iType==PlotGenerator::kBkgnd))
                histName += cfgLabel.histName;
...
bool parseInputs(int argc, char* argv[], ProgramOptions& options){
    ...
    if (argc < 2) {
        std::cerr << "Usage: \n";
        std::cerr << "\ttwops_plotter <results file name>" 
            "-o <output file name>\n"<< std::endl;
        return false;
    }
...
bool initializeAmpTools(ProgramOptions& options, 
    ...
    if (options.mcGen){
      ...
      plotResults = 
            std::make_unique<twops_PlotGen >(*results, PlotGenerator::kDefault);
    } 
    else {
      std::cout << "Plot results (MC gen off) \n";
      plotResults = 
            std::make_unique<twops_PlotGen >(*results, PlotGenerator::kNoGenMC);
    }
    ...
    for (size_t fileSet = 0; fileSet < reactionList.size(); ++fileSet){
        ...
        for (const auto& cfg : plotConfigs){
           ...
           generatePlots(options, plotResults, reactionName,
                   *plotFile, singleData, singleBkg, cfg, cfgLabel);
        }
        plotFile->Close();
    }
}
