...
typedef TwoLeptonGJPlotGenerator PlotGen;
...
int main( int argc, char* argv[] ){
  ...
  if (argc < 2){
    ...
    cout << "\ttwoleptonGJ_plotter <results file name> -o <output file name>" << endl << endl;
    return 0;
  }

  bool showGui = false;
  string outName = "twoleptonGJ_plot.root";
  ...
  for (unsigned int isum = 0; isum <= sums.size(); isum++){
    ...
    for (unsigned int iplot = 0; iplot < PlotGenerator::kNumTypes; iplot++){
      ...
      for (unsigned int ivar  = 0; ivar  < TwoLeptonGJPlotGenerator::kNumHists; ivar++){

        // set unique histogram name for each plot (could put in directories...)
        string histname =  "";
        if (ivar == TwoLeptonGJPlotGenerator::k2LeptonMass)  histname += "M2pi";
	else if (ivar == TwoLeptonGJPlotGenerator::kLeptonPCosTheta)  histname += "cosTheta";
        else if (ivar == TwoLeptonGJPlotGenerator::kPhi)  histname += "Phi";
        else if (ivar == TwoLeptonGJPlotGenerator::kphi)  histname += "phi";
        else if (ivar == TwoLeptonGJPlotGenerator::kPsi)  histname += "psi";
        else if (ivar == TwoLeptonGJPlotGenerator::kt)  histname += "t";
        else continue;
  ...
  ofstream outfile;
  outfile.open( "twoleptonGJ_fitPars.txt" );
