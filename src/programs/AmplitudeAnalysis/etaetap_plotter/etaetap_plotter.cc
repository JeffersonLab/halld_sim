...
typedef etaetapPlotGenerator etaetap_PlotGen;
...
// In 2 pseudoscalar case the refectivity is flipped in contrast to vec-ps case.
// See DocDB 4094

// KEEP THIS AFTER THE TESTS
...
ReflSum classifyReflSum(const std::string& sumName){
    if (sumName.find("NegativeRe") != std::string::npos ||
        sumName.find("PositiveIm") != std::string::npos){
        return ReflSum::Negative;  
    }
    else if (sumName.find("PositiveRe") != std::string::npos ||
             sumName.find("NegativeIm") != std::string::npos){
        return ReflSum::Positive; 
    }
    else {
        return ReflSum::Neither;
...
AmplitudeRegistry buildRegistry(std::unique_ptr<etaetap_PlotGen>& plotResults){    
...
void applyConfig(const PlotConfig& newConfig, const AmplitudeRegistry& reg, 
                  std::unique_ptr<etaetap_PlotGen>& plotResults){
...
// This function creates the plots that are assigned in etaetapPlotGenerator
void generatePlots(ProgramOptions& options, std::unique_ptr<etaetap_PlotGen>& plotResults,
    ...
    for (unsigned int iType = 0; iType < PlotGenerator::kNumTypes; ++iType){
        ...
        // Loop over all histograms defined in etaetapPlotGenerator
        ...
        for (etaetapPlotGenerator::Hist_index ivar = etaetapPlotGenerator::khm23; 
            ivar < etaetapPlotGenerator::kNumHists; 
            ivar = static_cast<etaetapPlotGenerator::Hist_index>(ivar + 1)){
            ...
            // and the translation table are defined in etaetapPlotGenerator 
            std::string histName = etaetapPlotGenerator::numToString(ivar);
...
bool parseInputs(int argc, char* argv[], ProgramOptions& options){
    ...
    if (argc < 2) {
        std::cerr << "Usage: \n";
        std::cerr << "\tetaetap_plotter <results file name>" 
            "-o <output file name>\n"<< std::endl;
        return false;
    }
...
bool initializeAmpTools(ProgramOptions& options, 
    ...
    if (options.mcGen){
      ...
      plotResults = 
            std::make_unique<etaetap_PlotGen >(*results, PlotGenerator::kDefault);
    } 
    else {
      std::cout << "Plot results (MC gen off) \n";
      plotResults = 
            std::make_unique<etaetap_PlotGen >(*results, PlotGenerator::kNoGenMC);
    }
