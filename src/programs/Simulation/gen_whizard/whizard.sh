...
echo 'source ~/Env/custom_GlueX_g4.sh' > $store/run.sh
echo "cd ${store}" >> $store/run.sh 
echo 'whizard run.sin' >> $store/run.sh
echo "lhe_to_root ${store}/" >> $store/run.sh
echo "mv NAME.root ../${file}.root" >> $store/run.sh 
echo "mv NAME.lhe ${file}.lhe" >> $store/run.sh 
echo "gzip ${file}.lhe" >> $store/run.sh 
echo "mv *.gz ../" >> $store/run.sh 
echo 'rm *.vg *.log *.la *.lo *.phs *.o *.mod *.f90 *.makefile' >> $store/run.sh
