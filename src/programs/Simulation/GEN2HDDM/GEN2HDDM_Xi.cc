...
int main(int narg, char *argv[])
{
   ...
   while (! infile->eof()) {
      ...
      *infile >> nParticles >> beamEnergy >> mechFlag;
      ...
      hddm_s::VertexList vs = rs().addVertices(5); //primary vertex and four detached vertices

      //Primary Vertex: vs(0)
      hddm_s::OriginList  os_primary = vs(0).addOrigins();
      hddm_s::ProductList ps_primary = vs(0).addProducts(3);//three final states: recoiling proton, baryon, anti-baryon
      ...
      //products
            ...
            //Baryon
             type = 0;                         // set the particel type as 0 (unknown), so that it won't decay again later
             ...
             ps_primary(1).setDecayVertex(1);
              ...
              float px_baryon, py_baryon, pz_baryon, E_baryon, d_baryon, t_baryon;
              px_baryon = 0;
              py_baryon = 0;
              pz_baryon = 0;
              E_baryon  = 0;
              d_baryon  = 0;
              t_baryon  = 0;
              hddm_s::OriginList  os_baryon = vs(1).addOrigins();//baryon detached vertex
              hddm_s::ProductList ps_baryon = vs(1).addProducts(2);

              for (int i= 0; i < 2; i++) //take care of baryon's decay products
              {
                if(i == 0)
                {
                    //Lambda
                     ...
                     ps_baryon(i).setType((Particle_t)type);
                     ps_baryon(i).setPdgtype(PDGtype((Particle_t)type));
                     ps_baryon(i).setId(4);           // unique value for this particle within the event 
                     ps_baryon(i).setParentid(2);     // All internally generated particles have no parent 
                     ps_baryon(i).setMech(mechFlag);         // maybe this should be set to something? 
                     ps_baryon(i).setDecayVertex(3);  //this particle later becomes vertex 3
                ...
                else if(i == 1) 
                {
                     ...
                     *infile >> dx >> dy >> dz;
                     ...
                     ps_baryon(i).setType((Particle_t)dtype);
                     ps_baryon(i).setPdgtype(PDGtype((Particle_t)dtype));
                     ps_baryon(i).setId(5);         // unique value for this particle within the event 
                     ps_baryon(i).setParentid(2);     // All internally generated particles have no parent 
                     ps_baryon(i).setMech(mechFlag);         //  
                     ps_baryon(i).setDecayVertex(0); //do not decay so set at 0

                     hddm_s::MomentumList pmoms = ps_baryon(i).addMomenta();
                     pmoms().setPx(dpx);
                     pmoms().setPy(dpy);
                     pmoms().setPz(dpz);
                     pmoms().setE(dE);
                     ...
                     d_baryon = sqrt( (dx-x)*(dx-x) + (dy-y)*(dy-y) + (dz-z)*(dz-z) );

                     os_baryon().setVx(dx-x);
                     os_baryon().setVy(dy-y);
                     os_baryon().setVz(dz-z);
                }
             ...
             float px_antibaryon, py_antibaryon, pz_antibaryon, E_antibaryon, d_antibaryon, t_antibaryon;
              px_antibaryon = 0;
              py_antibaryon = 0;
              pz_antibaryon = 0;
              E_antibaryon  = 0;
              d_antibaryon  = 0;
              t_antibaryon  = 0;
              hddm_s::OriginList  os_antibaryon = vs(2).addOrigins();//antibaryon detached vertex
              hddm_s::ProductList ps_antibaryon = vs(2).addProducts(2);

              for (int i= 0; i < 2; i++) //take care of antibaryon's decay products
              {
                if(i == 0)
                {
                    //Lambda
                     type = 0;                         // set the particel type as -18 (unknown), so that it won't decay again later
                     //Particle_t type1 = Lambda;
                     ps_antibaryon(i).setType((Particle_t)type);
                     ps_antibaryon(i).setPdgtype(PDGtype((Particle_t)type));
                     ps_antibaryon(i).setId(6);           // unique value for this particle within the event 
                     ps_antibaryon(i).setParentid(3);     // All internally generated particles have no parent 
                     ps_antibaryon(i).setMech(mechFlag);         // maybe this should be set to something? 
                     ps_antibaryon(i).setDecayVertex(4);  //this particle later becomes vertex 3
                ...
                else if(i == 1) 
                {
                     ...
                     *infile >> dx >> dy >> dz;
                     ...
                     ps_antibaryon(i).setType((Particle_t)dtype);
                     ps_antibaryon(i).setPdgtype(PDGtype((Particle_t)dtype));
                     ps_antibaryon(i).setId(7);         // unique value for this particle within the event 
                     ps_antibaryon(i).setParentid(3);     // All internally generated particles have no parent 
                     ps_antibaryon(i).setMech(mechFlag);         //  
                     ps_antibaryon(i).setDecayVertex(0); //do not decay so set at 0

                     hddm_s::MomentumList pmoms = ps_antibaryon(i).addMomenta();
                     pmoms().setPx(dpx);
                     pmoms().setPy(dpy);
                     pmoms().setPz(dpz);
                     pmoms().setE(dE);
                     ...
                     d_antibaryon = sqrt( (dx-x)*(dx-x) + (dy-y)*(dy-y) + (dz-z)*(dz-z) );

                     os_antibaryon().setVx(dx-x);
                     os_antibaryon().setVy(dy-y);
                     os_antibaryon().setVz(dz-z);
                }
      ...
      //for the last two detached vertices
             //Lambda Vertex: vs(3)
              ...
              d_lambda  = 0;
              t_lambda  = 0;
              hddm_s::OriginList  os_lambda = vs(3).addOrigins();
              hddm_s::ProductList ps_lambda = vs(3).addProducts(2);//three final states: recoiling proton, lambda, anti-lambda
              ...
              for (int i= 0; i < 2; i++) 
              {
                 ...
                 *infile >> dx >> dy >> dz;
                 ...
                 ps_lambda(i).setId(i+8);         // unique value for this particle within the event 
                 ps_lambda(i).setParentid(4);     // All internally generated particles have no parent 
                 ps_lambda(i).setMech(mechFlag);         //  
                 ps_lambda(i).setDecayVertex(0);  //do not decay anymore
              ...
              //now take care of the kinematic of lambda:
              hddm_s::MomentumList pmoms_lambda = ps_baryon(0).addMomenta();
              pmoms_lambda().setPx(px_lambda);
              pmoms_lambda().setPy(py_lambda);
              pmoms_lambda().setPz(pz_lambda);
              pmoms_lambda().setE(E_lambda);

              //now take care of the kinematic of baryon = lambda + pi-:
              px_baryon += px_lambda;
              py_baryon += py_lambda;
              pz_baryon += pz_lambda;
              E_baryon  += E_lambda;
              hddm_s::MomentumList pmoms_baryon = ps_primary(1).addMomenta();
              pmoms_baryon().setPx(px_baryon);
              pmoms_baryon().setPy(py_baryon);
              pmoms_baryon().setPz(pz_baryon);
              pmoms_baryon().setE(E_baryon);

              //now calculate time of baryon, then lambda: 
              //t = 0
              //t_baryon = t + d_baryon/v_baryon
              t_baryon = t + d_baryon*E_baryon/(29.9792458*sqrt(px_baryon*px_baryon+py_baryon*py_baryon+pz_baryon*pz_baryon));
              os_baryon().setT(t_baryon);
              //t_lambda = t_baryon + t_lambda/v_lambda
              t_lambda = t_baryon + d_lambda*E_lambda/(29.9792458*sqrt(px_lambda*px_lambda+py_lambda*py_lambda+pz_lambda*pz_lambda));
             ...
             //anti-Lambda Vertex: vs(4)
              ...
              d_lambdabar  = 0;
              t_lambdabar  = 0;
              hddm_s::OriginList  os_lambdabar = vs(4).addOrigins();
              hddm_s::ProductList ps_lambdabar = vs(4).addProducts(2);//three final states: recoiling proton, lambda, anti-lambda
              ...
              for (int i= 0; i < 2; i++) 
              {
                 ...
                 *infile >> dx >> dy >> dz;
                 ...
                 ps_lambdabar(i).setId(i+10);         // unique value for this particle within the event 
                 ps_lambdabar(i).setParentid(6);     // All internally generated particles have no parent 
                 ps_lambdabar(i).setMech(mechFlag);         // maybe this should be set to something? 
                 ps_lambdabar(i).setDecayVertex(0);  // do not decay anymore
              ...
              // First get the kinematics of lambdabar:
              hddm_s::MomentumList pmoms_lambdabar = ps_antibaryon(0).addMomenta();
              ...
              //now take care of the kinematic of baryon = lambda + pi-:
              px_antibaryon += px_lambdabar;
              py_antibaryon += py_lambdabar;
              pz_antibaryon += pz_lambdabar;
              E_antibaryon  += E_lambdabar;
              hddm_s::MomentumList pmoms_antibaryon = ps_primary(2).addMomenta();
              pmoms_antibaryon().setPx(px_antibaryon);
              pmoms_antibaryon().setPy(py_antibaryon);
              pmoms_antibaryon().setPz(pz_antibaryon);
              pmoms_antibaryon().setE(E_antibaryon);

              //now calculate time of antibaryon, then lambdabar: 
              //t = 0
              //t_antibaryon = t + d_antibaryon/v_antibaryon
              t_antibaryon = t + d_antibaryon*E_antibaryon/(29.9792458*sqrt(px_antibaryon*px_antibaryon+py_antibaryon*py_antibaryon+pz_antibaryon*pz_antibaryon));
              os_antibaryon().setT(t_antibaryon);
              //t_antilambda = t_antibaryon + t_antilambda/v_antilambda
              t_lambdabar = t_antibaryon + d_lambdabar*E_lambdabar/(29.9792458*sqrt(px_lambdabar*px_lambdabar+py_lambdabar*py_lambdabar+pz_lambdabar*pz_lambdabar));
