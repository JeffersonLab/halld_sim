...
int main( int argc, char* argv[] ){
	...
	// ??????????????????
	// need to get this from config file somehow????
	// ??????????????????
	float Mpip=ParticleMass(PiPlus), Mpi0=ParticleMass(Pi0);
	...
	double vecMass = 0.782;
	double vecWidth = 0.008;
	double bachMass = 0.135;
	childMasses.push_back(vecMass);
	threshold = bachMass + vecMass;
	...
	while( eventCounter < nEvents ){
		...
		// decay vector (and Delta++, if generated)
		ati.clearEvents();
		int i=0;
		while( i < batchSize ){
			
			double vec_mass_bw = m_bwGen[0]().first;
                        if( vec_mass_bw < 0.45 || vec_mass_bw > 0.86 ) continue;
			...
			if(bwGenLowerVertex.size() == 1) {
			  ...
			  // decay step for vector
        		  NBodyPhaseSpaceFactory vec_to_ps = NBodyPhaseSpaceFactory( vec_mass_bw, part_masses2);
