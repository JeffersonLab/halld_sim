...
int main( int argc, char* argv[] ){
	...
	bool diag = false;
	bool genFlat = false;
        Double_t kEta = ParticleMass(Eta);
	
	double beamMaxE   = 12.0;
	double beamPeakE  = 9.0;
	double beamLowE   = kEta;
	...
	AmpToolsInterface::registerAmplitude( EtaPb_tdist() );
	...
	// generate single Eta production
        Double_t Bslope = 230;
	GammaZToXZ phasespace( kEta, beamConfigFile, Bslope);
	
	vector< int > pTypes;
	pTypes.push_back( Gamma );
	pTypes.push_back( Eta );
	pTypes.push_back( Pb208 );
		...
		for( int i = 0; i < batchSize; ++i ){
			...
			if( !diag ){
				...
				if( weightedInten > rand || genFlat ){
					...
					cout << " gen_EtaPb: eventCounter=" << eventCounter << endl;
