...
int main( int argc, char* argv[] ){
  ...
  for (int i = 1; i < argc; i++){
    ...
    if (arg == "-e") {
      if ((i+1 == argc) || (argv[i+1][0] == '-')) arg = "-h";
      else startEventNum = atoi(argv[++i]);
    }
    if (arg == "-h"){
      ...
      cout << "\t -e        <event number>" << endl;
      cout << "\t           Starting event number for generated events (only affects HDDM outputs) [optional, default: 1]" << endl;
      cout << endl;
      exit(1);
    }
  }
  ...
  if( hddmName.size() != 0 ) hddmOut = new HDDMDataWriter( hddmName, runNum, seed, startEventNum);
