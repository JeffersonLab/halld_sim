...
{   
	...
	if(detectors_to_load == "all") {
		...
		smearers[SYS_TRD] = static_cast<Smearer*>(new GEMTRDSmearer(loop,config));
	} else {
	  ...
	  //cout << "Loading detector system "<< the_detector << ":" << token << " " << endl;
			switch(the_detector) {
				...
				case SYS_TRD:  smearers[the_detector] = static_cast<Smearer*>(new GEMTRDSmearer(loop,config));  break;
