...
int main( int argc, char* argv[] ) {
  ...
  if (configfile.size() != 0) {
    ...
    cobrem_vs_E->GetXaxis()->SetRangeUser(beamLowE, beamHighE);
  }
  
  TFile* diagOut = new TFile( "gen_compton_simple_diagnostic.root", "recreate" );
  ...
  TH2F* h_Tkin_electron_vs_egam = new TH2F("Tkin_electron_vs_egam", ";E_{#gamma} [GeV];T^{kin}_{e^{-}} [GeV];Count [a.u.]", 1000, 0.0, 12.0, 1000, 0.0, 12.0);
  TH2F* h_theta_photon_vs_egam = new TH2F("theta_photon_vs_egam", ";E_{#gamma} [GeV];#theta_{#gamma} [GeV];Count [a.u.]", 1000, 0.0, 12.0, 1000, 0., 10.);
  TH2F* h_theta_electron_vs_egam = new TH2F("theta_electron_vs_egam", ";E_{#gamma} [GeV];#theta_{e^{-}} [GeV];Count [a.u.]", 1000, 0.0, 12.0, 1000, 0., 10.);
  ...
  for (int i = 0; i < nEvents; ++i) {
    ...
    h_Tkin_electron_vs_egam->Fill(ebeam, electron_4v.E() - electron_4v.M());
    h_theta_electron_vs_egam->Fill(ebeam, electron_4v.Theta() * TMath::RadToDeg());
    h_Tkin_photon_vs_egam->Fill(ebeam, gamma_4v.E() - gamma_4v.M());
    h_theta_photon_vs_egam->Fill(ebeam, gamma_4v.Theta() * TMath::RadToDeg());
  ...
  h_Tkin_electron_vs_egam->Write();
  h_theta_photon_vs_egam->Write();
  h_theta_electron_vs_egam->Write();
  //cobrem_vs_E->Write();
  diagOut->Close();
