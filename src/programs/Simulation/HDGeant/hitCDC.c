      ...
      int track, int stack, int history, int ipart )
{
   ...
   if (!initialized) {
      ...
      if (fabs(B[2])>1e-3){
         ...
         nvalues=2;
         status = GetColumn("CDC/drift_parameters", &nvalues, values, "B1");
         if (status != 0) {
            printf("CDC: drift_parameters table corrupted in database!\n");
         }
         BSCALE_PAR1 = values[0];

         nvalues=2;
         status = GetColumn("CDC/drift_parameters", &nvalues, values, "B2");
         if (status != 0) {
            printf("CDC: drift_parameters table corrupted in database!\n");
         }
         BSCALE_PAR2 = values[0];
      }
      else{
         nvalues=78;
