...
public:
     ...
     * - index 0 (B): beam photon
     * - lower_vertex_indices: lower vertex particles, e.g. proton, pi+ that decay from Delta++
     * - All leftover indices: final state particles from the upper vertex
     *
     * @param[in] filename path to the .fit file
     ...
     * - index 0 (B): beam photon
     * - lower_vertex_indices: lower vertex particles, e.g. proton, pi+ that decay from Delta++
     * - isobar_indices: isobar particles, e.g. pi+ pi- from rho
     * - all indices not in lower_vertex_indices or 0: final state particles from upper vertex
     *
     * @todo: implement. make sure no conflicts with lower vertex indices. Also this
     * should add isobar mass and event counts to the output. Usually one is interested
     * in the distribution though, rather than just the bin centers/edges like we are
     * for the total binned mass. For now then, we will leave this as a future
     * improvement until a use case arises that requires it.
     *
     * @param[in] filename path to the .fit file
     * @param[in] lower_vertex_indices 4-momenta indices for lower vertex particles
