...
///////////////////////////////////////////////////////////////////////////////
__global__ void
GPULowerVertexDelta_kernel( GPU_AMP_PROTO, int m_d, int m_p, int m_c )
{
	...
	if ( m_c == 1 )
		amp = 2/3.* wignerD( 3/2, lambda_Delta, lambda_proton, cosTheta, phi );
	if ( m_c == -1 )
		amp = 2/3.* Conjugate( wignerD( 3/2, lambda_Delta, lambda_proton, cosTheta, phi ) );
...
void
GPULowerVertexDelta_exec( dim3 dimGrid, dim3 dimBlock, GPU_AMP_PROTO, int m_d, int m_p, int m_c )

{

	GPULowerVertexDelta_kernel<<< dimGrid, dimBlock >>>
		( GPU_AMP_ARGS, m_d, m_p, m_c );
