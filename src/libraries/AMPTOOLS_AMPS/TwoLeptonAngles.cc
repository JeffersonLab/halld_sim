...
TwoLeptonAngles::TwoLeptonAngles( const vector< string >& args ) :
  UserAmplitude< TwoLeptonAngles >( args )
{
  ...
  //    Usage: amplitude <reaction>::<sum>::<ampName> TwoLeptonAngles <rho000> ... <rho1m12> <polAngle> <polFraction>
  if(args.size() == 11) {
  ...
  //    Usage: amplitude <reaction>::<sum>::<ampName> TwoLeptonAngles <rho000> ... <rho1m12> <polAngle> <rootFile> <hist>
...
complex< GDouble >
TwoLeptonAngles::calcAmplitude( GDouble** pKin, GDouble* userVars ) const {
...
void
TwoLeptonAngles::calcUserVars( GDouble** pKin, GDouble* userVars ) const {
...
void
TwoLeptonAngles::launchGPUKernel( dim3 dimGrid, dim3 dimBlock, GPU_AMP_PROTO ) const {

  GPUTwoLeptonAngles_exec( dimGrid, dimBlock, GPU_AMP_ARGS,
