...
__global__ void
GPUTwoPiAngles_Delta_kernel( GPU_AMP_PROTO, GDouble rho000, GDouble rho100,
                        GDouble rho1m10, GDouble rho111, GDouble delta_rho133,
		        GDouble rho101, GDouble rho1m11, GDouble rho102,
		        GDouble rho1m12, GDouble polAngle, GDouble delta_rho111 ){
  ...
  GDouble bigPhi = GPU_UVARS(4);   // before polAngle*0.017453293 + 
  GDouble phi = GPU_UVARS(5);
  GDouble rho001=2*(delta_rho111 + delta_rho133 - rho111);           
...
{  
  ...
  GPUTwoPiAngles_Delta_kernel<<< dimGrid, dimBlock >>>
    ( GPU_AMP_ARGS, rho000, rho100, rho1m10, rho111,delta_rho133,
      rho101, rho1m11, rho102, rho1m12, polAngle, delta_rho111 );
}
