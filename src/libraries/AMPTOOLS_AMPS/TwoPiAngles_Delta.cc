...
TwoPiAngles_Delta::TwoPiAngles_Delta( const vector< string >& args ) :
  UserAmplitude< TwoPiAngles_Delta >( args )
{
  ...
  delta_rho111 = AmpParameter( args[12] );
  constraint = true;
...
complex< GDouble >
TwoPiAngles_Delta::calcAmplitude( GDouble** pKin, GDouble* userVars ) const {
...
GDouble Pgamma = userVars[kPgamma];
 ...
 W -= Pgamma*cos(2.*bigPhi) * (rho111*sinSqTheta + rho001_val*cosTheta*cosTheta - sqrt(2.)*rho101*sin2Theta*cos(phi) - rho1m11*sinSqTheta*cos(2.*phi)); 
...
void
TwoPiAngles_Delta::calcUserVars( GDouble** pKin, GDouble* userVars ) const {
...
void
TwoPiAngles_Delta::launchGPUKernel( dim3 dimGrid, dim3 dimBlock, GPU_AMP_PROTO ) const {

  GPUTwoPiAngles_Delta_exec( dimGrid, dimBlock, GPU_AMP_ARGS,
			   rho000, rho100, rho1m10,
			   rho111, delta_rho133, rho101,
			   rho1m11, rho102, rho1m12,
			   polAngle, delta_rho111 );
}
