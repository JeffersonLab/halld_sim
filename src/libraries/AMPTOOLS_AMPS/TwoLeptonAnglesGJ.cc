...
TwoLeptonAnglesGJ::TwoLeptonAnglesGJ( const vector< string >& args ) :
  UserAmplitude< TwoLeptonAnglesGJ >( args )
{
  ...
  //    Usage: amplitude <reaction>::<sum>::<ampName> TwoLeptonAnglesGJ <rho000> ... <rho1m12> <polAngle> <polFraction>
  if(args.size() == 11) {
  ...
  //    Usage: amplitude <reaction>::<sum>::<ampName> TwoLeptonAnglesGJ <rho000> ... <rho1m12> <polAngle> <rootFile> <hist>
...
complex< GDouble >
TwoLeptonAnglesGJ::calcAmplitude( GDouble** pKin, GDouble* userVars ) const {
...
void
TwoLeptonAnglesGJ::calcUserVars( GDouble** pKin, GDouble* userVars ) const {
  ...
  // choose GJ frame: z-axis is defined by the incoming photon in V rest frame
...
void
TwoLeptonAnglesGJ::launchGPUKernel( dim3 dimGrid, dim3 dimBlock, GPU_AMP_PROTO ) const {

  GPUTwoLeptonAnglesGJ_exec( dimGrid, dimBlock, GPU_AMP_ARGS,
