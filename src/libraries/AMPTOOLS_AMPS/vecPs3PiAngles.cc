...
#include "TMath.h"
#include "vecPs3PiAngles.h"
#include "IUAmpTools/AmpToolsInterface.h"
#include "IUAmpTools/report.h"

static const char* kModule = "vecPs3PiAngles";
...
vector <double> getVectorDecayAngles( const TLorentzVector& beamPLab,
  ...
  // y-axis is normal to the plane with the parent and beam vectors in the X rest frame
  TVector3 z = (parent_x.Vect()).Unit();
  TVector3 y = ((beam_x.Vect()).Cross(z)).Unit();
...
vector<double> getXDecayAngles( double polAngle, 
    ...
    // y-axis is normal to the production plane defined by particle X and the beam
    //        Both vectors are expressed in the Lab frame
    //        (the same as helicity frame)
     TLorentzVector beam_x = beam_cm;
     beam_x.Boost(-1.0*xBoost);
     TVector3 z = (beam_x.Vect()).Unit();
     TVector3 y = ((beamLab.Vect()).Cross(particleXLab.Vect())).Unit();
     TVector3 x = (y.Cross(z)).Unit();
    ...
    // vector and the normal to the production plane
