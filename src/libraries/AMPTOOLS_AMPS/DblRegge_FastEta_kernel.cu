...
#include "DblReggeHelper_FastEta.cuh"
//#include "AMPTOOLS_AMPS/DblReggeHelper.cuh"

__global__ void
DblRegge_FastEta_kernel(GPU_AMP_PROTO, GDouble S0, GDouble b_eta, int charge){
...
//	GDouble t2 = GPU_UVARS(3);
	...
	WCUComplex amp =  GPU_calcAmplitude(s, s12, s23, t1, u3, S0, b_eta, beamM2, p1M2, p2M2, recoilM2, charge);
...
void GPUDblRegge_FastEta_exec( dim3 dimGrid, dim3 dimBlock, GPU_AMP_PROTO,
		GDouble S0, GDouble b_eta,int charge  )
{

	DblRegge_FastEta_kernel<<< dimGrid, dimBlock >>>( GPU_AMP_ARGS, S0, b_eta, charge );
}
