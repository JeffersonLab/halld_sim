#ifndef CUDA_DBLREGGEHELPER_FASTPI
#define CUDA_DBLREGGEHELPER_FASTPI
...
static __device__ WCUComplex GPU_ampEtaPi0(GDouble par, int hel[3], GDouble inv[5], GDouble mass2[4], GDouble b_pi, GDouble S0, int charge ){
...
if(charge ==0){
	...
	WCUComplex ADR2 = GPU_DoubleRegge(tau, s, si, alp,S0); // fast pi0
	// helicity part
	...
	GDouble Bot2 = exp(abs(b_pi)*t2);

	WCUComplex finalFactor = fac3*(Bot2*fac2*ADR2 );
	return finalFactor;
}
...
static __device__ WCUComplex GPU_calcAmplitude(GDouble s, GDouble s12, GDouble s23, GDouble t1, GDouble u3, GDouble S0, GDouble b_pi, GDouble beamM2, GDouble p1M2, GDouble p2M2, GDouble recoilM2, int charge){
	...
	WCUComplex amp = GPU_ampEtaPi0(param, hel, inv, mass2, b_pi, S0, charge);
