...
//  cout << "decay vector: (not mine)" << endl;
//  decayVector.Unit().Print();
 
  TVector3 components(decayVector.Dot(x),decayVector.Dot(y),decayVector.Dot(z));

//  cout << "Vector helicity angles (not mine):" << endl;
//  components.Unit().Print();
...
/*
    cout << "daughter_x_unit: (omega in X frame)" << endl;
    daughter_x.Print();
    daughter_x_unit.Print();
...
*/
    ...
    TVector3 altEps( 1.0, 0.0, 0.0 );
//    double altBigPhi = atan2( y.Dot( altEps ), beamLab.Vect().Unit().Dot( altEps.Cross( y ) ) );
//    cout << "altBigPhi = " << altBigPhi << endl;
