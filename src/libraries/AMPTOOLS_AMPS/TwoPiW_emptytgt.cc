...
TwoPiW_emptytgt::calcAmplitude( GDouble** pKin ) const
{
  ...
  complex<GDouble> ImagOne(0,1);
    ...
    // Aw = exp( -(Wpipi-Gmean)*(Wpipi-Gmean)/(2*Gsigma*Gsigma)) + ImPart*ImagOne;   // Gauissian: let phase space cut off low 2piM
    Aw = sqrt(1*exp( -(Wpipi-Gmean)/(Gsigma)) + 0.6 * exp( -(Wpipi-Gmean)/(4*Gsigma))
	      + ImPart*ImagOne );   // Testing new parameter
    // Aw = exp( -(Wpipi-Gmean)/(2*Gsigma)) + ImPart*ImagOne ;    // Exponential: factor of 1/2 is for amplitude
    // Aw = Wpipi-Gmean>0? 10*exp( -(Wpipi-Gmean)/(2*Gsigma)) + ImPart*ImagOne : 0;    // Exponential: factor of 1/2 is for amplitude
