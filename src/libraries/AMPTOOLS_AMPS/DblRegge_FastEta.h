#if !defined(DBLREGGE_FASTETA)
#define DBLREGGE_FASTETA
...
void GPUDblRegge_FastEta_exec(dim3 dimGrid, dim3 dimBlock, GPU_AMP_PROTO, GDouble S0, GDouble b_eta, int charge );
...
class DblRegge_FastEta : public UserAmplitude< DblRegge_FastEta >
{

public:

        DblRegge_FastEta() : UserAmplitude< DblRegge_FastEta >() { };
        DblRegge_FastEta( const vector< string >& args );

        string name() const { return "DblRegge_FastEta"; }
	...
	complex< GDouble > calcAmplitude( GDouble** pKin, GDouble* userVars ) const;
        ...
        std::complex<double> DoubleRegge(int tau[2], double s, double sie[2], double alpe[2]) const;
