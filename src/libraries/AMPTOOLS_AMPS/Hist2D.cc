...
Hist2D::calcAmplitude( GDouble** pKin ) const {
	...
	int bin = hist2D->FindBin(userVarX, userVarY); // generic bin index from 2D histogram
	GDouble W = hist2D->GetBinContent(bin); // weighted model of intensity from histogram 
