#if !defined(TWOLEPTONANGLES)
#define TWOLEPTONANGLES
...
class TwoLeptonAngles : public UserAmplitude< TwoLeptonAngles >
{
    
public:
	
	TwoLeptonAngles() : UserAmplitude< TwoLeptonAngles >() { };
	TwoLeptonAngles( const vector< string >& args );

	enum UserVars { kPgamma = 0, kCosTheta, kSinSqTheta, kSin2Theta,
			kBigPhi, kPhi, kNumUserVars };
	unsigned int numUserVars() const { return kNumUserVars; }

	string name() const { return "TwoLeptonAngles"; }
