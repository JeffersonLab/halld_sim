...
complex< GDouble > Flatte::calcAmplitude( GDouble** pKin, GDouble* userData ) const {
   ...
   GDouble curMass = (P1+P2).M();
   complex<GDouble> imag(0.,1.);

   complex<GDouble> gamma11 = (GDouble)m_g1 * Flatte::breakupMom( curMass, m_mass11, m_mass12 );
   complex<GDouble> gamma22 = (GDouble)m_g2 * Flatte::breakupMom( curMass, m_mass21, m_mass22 );

   complex<GDouble> gammaLow;
   if( (m_mass11+m_mass12) < (m_mass21+m_mass22) ) gammaLow = gamma11;
   else gammaLow = gamma22;

   complex<GDouble> gamma_j;
   ...
   complex<GDouble>  result = (GDouble)m_mass * sqrt( gammaLow*gamma_j ) / ( (GDouble)m_mass*(GDouble)m_mass - curMass*curMass - imag * (GDouble)m_mass * (gamma11+gamma22) );
   return result;
}
...
complex<GDouble> Flatte::phaseSpaceFac(GDouble m, GDouble mDec1, GDouble mDec2) const{

   complex<GDouble> result(0.,0.);
   ...
   GDouble termPlus=(mDec1+mDec2)/m;
   GDouble termMinus=(mDec1-mDec2)/m;
   GDouble tmpVal=(1.-termPlus*termPlus) * (1.-termMinus*termMinus);
   if(tmpVal>=0.) result = complex<GDouble>(std::sqrt(tmpVal), 0.);
   else result = complex<GDouble>(0., sqrt(-tmpVal));   
   return result;
}

template<typename mType>
complex<GDouble> Flatte::breakupMom(mType m, GDouble mDec1, GDouble mDec2) const{
   complex<GDouble> result = Flatte::phaseSpaceFac(m, mDec1, mDec2)*m/GDouble(2.);
   return result;  
}
