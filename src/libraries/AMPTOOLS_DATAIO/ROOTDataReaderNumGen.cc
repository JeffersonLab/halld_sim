...
#include "AMPTOOLS_DATAIO/ROOTDataReaderNumGen.h"
...
ROOTDataReaderNumGen::ROOTDataReaderNumGen( const vector< string >& args ):
  UserDataReader< ROOTDataReaderNumGen >( args ),
  m_eventCounter( 0 ),
  m_useWeight( false )
{
...
ROOTDataReaderNumGen::~ROOTDataReaderNumGen()
{
  if( m_inFile != NULL ) m_inFile->Close();
}

void
ROOTDataReaderNumGen::resetSource()
{
...
ROOTDataReaderNumGen::getEvent()
{
  if( m_eventCounter < 1 ){
    ...
    if(m_useWeight) {   
      ...
      sumW = m_inTree->GetEntries();
    }

    return new Kinematics( particleList, sumW );
  }
  else{
