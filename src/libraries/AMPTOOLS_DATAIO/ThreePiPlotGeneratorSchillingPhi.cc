...
#include "AMPTOOLS_DATAIO/ThreePiPlotGeneratorSchillingPhi.h"
#include "AMPTOOLS_AMPS/wignerD.h"
#include "IUAmpTools/Histogram1D.h"
#include "IUAmpTools/Kinematics.h"

ThreePiPlotGeneratorSchillingPhi::ThreePiPlotGeneratorSchillingPhi( const FitResults& results ) :
...
ThreePiPlotGeneratorSchillingPhi::ThreePiPlotGeneratorSchillingPhi( ) :
PlotGenerator( )
{
...
void ThreePiPlotGeneratorSchillingPhi::createHistograms( ) {
  
  // calls to bookHistogram go here
  bookHistogram( kPhi1020Mass, new Histogram1D( 200, 0.9, 1.2, "MPhi", "Invariant Mass;M_{3#pi}") );
  bookHistogram( kY20, new Histogram1D( 200, 0.9, 1.2, "Y20", "Y^{0}_{2}(#theta); M_{3#pi}") );
  bookHistogram( kReY21Psi, new Histogram1D( 200, 0.9, 1.2, "ReY21Psi", "Re(Y^{1}_{2}(#theta,#psi)); M_{3#pi}") );
  bookHistogram( kReY22Psi, new Histogram1D( 200, 0.9, 1.2, "ReY22Psi", "Re(Y^{2}_{2}(#theta,#psi)); M_{3#pi}") );
  bookHistogram( kReY21Phi, new Histogram1D( 200, 0.9, 1.2, "ReY21Phi", "Re(Y^{1}_{2}(#theta,#phi)); M_{3#pi}") );
  bookHistogram( kReY22Phi, new Histogram1D( 200, 0.9, 1.2, "ReY22Phi", "Re(Y^{2}_{2}(#theta,#phi)); M_{3#pi}") );
  bookHistogram( kY40, new Histogram1D( 200, 0.9, 1.2, "Y40", "Y^{0}_{4}(#theta); M_{3#pi}") );
  
  bookHistogram( kCosTheta, new Histogram1D( 50, -1, 1, "CosTheta", "cos#theta_{H};cos#theta_{H}" ) );
  bookHistogram( kPhi, new Histogram1D( 50, -1*PI, PI, "phi", "#varphi_{H}; #varphi_{H}" ) );
  bookHistogram( kCosThetaPhi, new Histogram2D( 50, -1*PI, PI, 50, -1, 1, "CosThetaPhi", "Helicity Frame; #varphi_{H}; cos#theta_{H}" ) );
  ...
  bookHistogram( kPhiPiPlus,  new Histogram1D( 50, -1*PI, PI, "PhiPiPlus",  "#varphi_{lab} of #pi^{+}" ) );
  bookHistogram( kPhiPiMinus, new Histogram1D( 50, -1*PI, PI, "PhiPiMinus", "#varphi_{lab} of #pi^{-}" ) );
  bookHistogram( kPhiPi0, new Histogram1D( 50, -1*PI, PI, "PhiPi0", "#varphi_{lab} of #pi^{0}" ) );
  
  bookHistogram( kt, new Histogram1D( 100, 0, 1.0 , "t", "-t" ) );
}

void
ThreePiPlotGeneratorSchillingPhi::projectEvent( Kinematics* pKin, const string& reactionName ){
  ...
  // this class is written in the context of phi1020 -> 3 pi
  ...
  TLorentzVector phi1020 = p1 + p2 + p3;
  
  // construct a boost that will take us to the phi1020 rest frame:
  TLorentzRotation phi1020RestBoost( -phi1020.BoostVector() );
  
  TLorentzVector recoil_phi1020Rest = phi1020RestBoost * recoil;
  TLorentzVector p1_phi1020Rest = phi1020RestBoost * p1;
  TLorentzVector p2_phi1020Rest = phi1020RestBoost * p2;
  
  // for the 3pi decay the normal to the decay plane is the
  // key vector that describes the decay
  TVector3 norm = (p2_phi1020Rest.Vect().Cross(p1_phi1020Rest.Vect())).Unit();
  ...
  // as the direction of the recoil in the phi1020 rest frame, so this
  // works fine:
  TVector3 zHel = -1*recoil_phi1020Rest.Vect().Unit();
  ...
  // to the CM or phi1020 rest frame as all of these boost vectors
  // lie in the production plane
  TVector3 yHel = (beam.Vect().Cross(phi1020.Vect())).Unit();
  ...
  // will be filled
  vector< double > phi1020Mass;
  phi1020Mass.push_back( phi1020.M() );
  
  // calls to fillHistogram go here
  
  fillHistogram( kPhi1020Mass, phi1020.M() );
  fillHistogram( kY20, phi1020Mass, real( Y( 2, 0, cosTheta, psi ) ) );
  fillHistogram( kReY21Psi, phi1020Mass, real( Y( 2, 1, cosTheta, psi ) ) );
  fillHistogram( kReY22Psi, phi1020Mass, real( Y( 2, 2, cosTheta, psi ) ) );
  fillHistogram( kReY21Phi, phi1020Mass, real( Y( 2, 1, cosTheta, phi ) ) );
  fillHistogram( kReY22Phi, phi1020Mass, real( Y( 2, 2, cosTheta, phi ) ) );
  fillHistogram( kY40, phi1020Mass, real( Y( 4, 0, cosTheta, psi ) ) );
