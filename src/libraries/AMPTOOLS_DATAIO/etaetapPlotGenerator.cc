...
etaetapPlotGenerator::etaetapPlotGenerator( const FitResults& results, Option opt ) :
...
etaetapPlotGenerator::etaetapPlotGenerator( ) :
PlotGenerator( )
{
	createHistograms();
}

void etaetapPlotGenerator::createHistograms( ){
  ...
  bookHistogram( khm23, new Histogram1D( 100, 1.4, 3.2, "hm23", "M(#eta#eta') [GeV/c^{2}]" ) );
...
void etaetapPlotGenerator::projectEvent( Kinematics* kin, const string& reactionName ){
  ...
  TLorentzVector P3 = kin->particle(3); //etaprime
  ...
  // Angles of etaetap system in LAB frame:
