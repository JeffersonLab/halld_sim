      	...
      	if (m_inTree){
			...
			while( m_eventCounter < static_cast< unsigned int >( m_inTree->GetEntries() ) ){
				...
				for(unsigned int i = 0; i < m_numParticles; i++){	
					...
					if (i > 0 && i < m_numParticles-1) finalstate += TLorentzVector( m_PxP[i], m_PyP[i], m_PzP[i], m_EnP[i] );
					if (i == 0 || i == m_numParticles-1) recoil += TLorentzVector( m_PxP[i], m_PyP[i], m_PzP[i], m_EnP[i] ); // proton (particle 0) and last particle in the list compose the recoil particle. Eventually need to modify this to allow for stable lower vertices
				}
